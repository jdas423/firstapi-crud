...
from .. import schemas, utilis
from app.oauth import AuthJWT
...
async def create_user(payload: schemas.CreateUserSchema, request: Request):
    ...
    #  Hash the password
    payload.password = utilis.hash_password(payload.password)
    del payload.confirmpassword
    payload.email = payload.email.lower()
    payload.dateCreated = datetime.utcnow()
    payload.lastUpdatedDate = payload.dateCreated
...
def login(payload: schemas.LoginUserSchema, response: Response, Authorize: AuthJWT = Depends()):
    ...
    if not utilis.verify_password(payload.password, user['password']):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail='Incorrect Email or Password')
...
def logout(response: Response, Authorize: AuthJWT = Depends(), user_id: str = Depends(oauth.require_user)):
